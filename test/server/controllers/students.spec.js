...
describe('/api/students', () => {
  ...
  test('GET /?period', async done => {
    ...
    const res = await chai
      .request(server.listen())
      .get('/api/students/?period=2018.4')
      .set('Authorization', `Bearer ${token}`)
    expect(res.status).toEqual(200)
    expect(res.type).toEqual('application/json')
    expect(res.body).toBeDefined()
    expect(res.body.length).toEqual(3)
    expect(res.body[0].name).toEqual('ENZO FERREIRA ALVES')
    expect(res.body[0].period).toEqual('2018.4')
    expect(res.body[1].name).toEqual('KAUAN CARVALHO SANTOS')
    expect(res.body[1].period).toEqual('2018.4')
    expect(res.body[2].name).toEqual('EDUARDO ALVES LIMA')
    expect(res.body[2].period).toEqual('2018.4')
    done()
  })
})
